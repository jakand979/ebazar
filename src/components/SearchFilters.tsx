...
const SearchFilters = (props: SearchFiltersProps) => {
    const {searchText, onFilterChange} = props;
    ...
    useEffect(() => {
        const fetchMinMaxPrice = async() => {
            const res = await axios.get("http://localhost:8080/products/search-min-max/" + searchText, {
                headers: {
    ...
    useEffect(() => {
        const fetchDistinctSizes = async() => {
            const res = await axios.get("http://localhost:8080/products/search-distinct-sizes/" + searchText, {
                headers: {
                    "Content-Type": "application/json"
                },
                params: {
        ...
        fetchDistinctSizes();
    }, [searchText, inputMinPrice, inputMaxPrice]);

    useEffect(() => {
        const fetchDistinctColors = async() => {
            const res = await axios.get("http://localhost:8080/products/search-distinct-colors/" + searchText, {
                headers: {
                    "Content-Type": "application/json"
                },
                params: {
    ...
    useEffect(() => {
        const fetchDistinctBrands = async() => {
            const res = await axios.get("http://localhost:8080/products/search-distinct-brands/" + searchText, {
                headers: {
                    "Content-Type": "application/json"
                },
                params: {
    ...
    return (
        <div className="flex flex-col w-1/5 h-fit ml-16 mt-16 mb-8 pt-4 pb-8 border-2">
            ...
            <div className="flex flex-col ml-6 mt-6">
                ...
                <h1 className="mr-auto mt-6 text-2xl">{minPrice == null ? 0 : minPrice}-{maxPrice == null ? 0 : maxPrice} PLN</h1>
            </div>
            <hr className="mt-6" />
            <div className="flex flex-col ml-6 mt-6">
                <h1 className="text-3xl font-montserrat font-semibold">Rozmiar</h1>
                <div className="flex flex-row flex-wrap items-center mt-4">
                    {distinctSizes.length === 0 ? "" : distinctSizes.map((size, index) => (
                        <div 
                            key={index}
                            onClick={() => {
                            ...
                            className={`flex justify-center items-center w-20 h-11 border-2 text-xl font-montserrat font-medium  hover:border-slate-700 cursor-pointer ${selectedSizes.includes(size) ? "bg-orange-700 text-white" : "bg-white text-black"}`}>
                            {size}
                        </div>
                    ))}
                </div>
            ...
            <div className="flex flex-col ml-6 mt-6">
                ...
                {distinctColors.length === 0 ? "" : distinctColors.map((color, index) => (
            ...
            <div className="flex flex-col ml-6 mt-6">
                ...
                {distinctBrands.length === 0 ? "" : distinctBrands.map((brand, index) => (
                    <div key={index} className="flex items-center mt-4">
                        <input 
                            type="checkbox" 
                            name="brand"
